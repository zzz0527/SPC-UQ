    ...
    def __init__(
        ...
        **kwargs
        ...
        if num_classes is not None:
            self.output_alpha = nn.Linear(nStages[3], num_classes)
            self.output_beta = nn.Linear(nStages[3], num_classes)
            self.output_nu = nn.Linear(nStages[3], num_classes)
            self.output_gamma = nn.Linear(nStages[3], num_classes)
...
def wrn_edl(temp=1.0, spectral_normalization=True, mod=True, **kwargs):
