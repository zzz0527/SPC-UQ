...
class VGG(nn.Module):
    def __init__(
        ...
        if mnist:
            ...
            self.features = self._make_layers(cfg_imagenet[vgg_name])
        else:
            self.inp_sizes = inp_size_cifar[vgg_name]
        ...
        if imagenet:
            ...
            self.output_alpha = nn.Linear(4096, num_classes)
            self.output_beta = nn.Linear(4096, num_classes)
            self.output_nu = nn.Linear(4096, num_classes)
            self.output_gamma = nn.Linear(4096, num_classes)
            self.feature = None

        else:
            self.output_alpha = nn.Linear(512, num_classes)
            self.output_beta = nn.Linear(512, num_classes)
            self.output_nu = nn.Linear(512, num_classes)
            self.output_gamma = nn.Linear(512, num_classes)
            self.feature = None
    ...
    def _make_layers(self, cfg):
        ...
        for i, x in enumerate(cfg):
            if x == "M":
                ...
                in_channels = x
        if self.imagenet:
            layers += [nn.AdaptiveAvgPool2d((7, 7))]  # 确保全连接层输入为 7x7
        else:
            layers += [nn.AvgPool2d(kernel_size=1, stride=1)]
        return nn.Sequential(*layers)


def vgg11_edl(spectral_normalization=True, mod=True, temp=1.0, mnist=False, imagenet=False, **kwargs):
    model = VGG("VGG11", spectral_normalization=spectral_normalization, mod=mod, temp=temp, mnist=mnist, imagenet=imagenet, **kwargs)
    return model


def vgg13_edl(spectral_normalization=True, mod=True, temp=1.0, mnist=False, imagenet=False, **kwargs):
    model = VGG("VGG13", spectral_normalization=spectral_normalization, mod=mod, temp=temp, mnist=mnist, imagenet=imagenet, **kwargs)
    return model


def vgg16_edl(spectral_normalization=True, mod=True, temp=1.0, mnist=False, imagenet=False, **kwargs):
    model = VGG("VGG16", spectral_normalization=spectral_normalization, mod=mod, temp=temp, mnist=mnist, imagenet=imagenet, **kwargs)
    return model


def vgg19_edl(spectral_normalization=True, mod=True, temp=1.0, mnist=False, imagenet=False, **kwargs):
    model = VGG("VGG19", spectral_normalization=spectral_normalization, mod=mod, temp=temp, mnist=mnist, imagenet=imagenet, **kwargs)
    return model
