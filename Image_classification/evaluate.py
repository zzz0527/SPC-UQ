...
from metrics.uncertainty_confidence import entropy, logsumexp, self_consistency_o, self_consistency_e, edl_unc, certificate
...
models = {"resnet50": resnet50, "resnet50_edl":resnet50_edl, "wide_resnet": wrn, "wide_resnet_edl": wrn_edl, "wide_resnet_uq": wrn_uq, "vgg16": vgg16, "vgg16_edl": vgg16_edl, "vgg16_uq": vgg16_uq, "inception":inception_v3, "imagenet_wide":imagenet_wide, "imagenet_vgg16":imagenet_vgg16, "imagenet_vit":imagenet_vit}

model_to_num_dim = {"resnet50": 2048, "resnet50_edl":2048, "wide_resnet": 640, "wide_resnet_edl": 640, "wide_resnet_uq": 640, "vgg16": 512, "vgg16_edl": 512, "vgg16_uq": 512, "inception": 2048, "imagenet_wide":2048, "imagenet_vgg16":4096, "imagenet_vit":768}

model_to_input_dim = {"resnet50": 32, "resnet50_edl": 32, "wide_resnet": 32, "wide_resnet_edl": 32, "wide_resnet_uq": 32, "vgg16": 32, "inception": 128, "vgg16_edl": 32, "vgg16_uq": 32, "imagenet_wide":224, "imagenet_vgg16":224, "imagenet_vit":224}

model_to_last_layer = {"resnet50": "module.fc", "wide_resnet": "module.linear", "vgg16": "module.classifier", "inception": "module.fc", "imagenet_wide": "module.linear", "imagenet_vgg16": "module.classifier", "imagenet_vit": "module.linear"}
...
if __name__ == "__main__":
    ...
    for i in range(args.runs):
        ...
        if args.model_type == "ensemble":
            ...
            (_, _, _), (_, _, _), ood_m1_auroc, ood_m1_auprc = get_roc_auc_logits(logits, ood_logits, self_consistency_o,
                                                                                  device)
            ...
            # (adv_conf_matrix, adv_accuracy, adv_labels_list, adv_predictions,
            #  adv_confidences,) = test_classification_net(net, adv_test_loader, device)

            adv_logits, adv_labels = get_logits_labels_uq(net, adv_test_loader, device)

            (_, _, _), (_, _, _), adv_m1_auroc, adv_m1_auprc = get_roc_auc_logits(logits, adv_logits, self_consistency_e, device)
            ...
            if (args.model_type == "gmm"):
                ...
                if os.path.exists(spc_model_path):
                    ...
                    if args.crossval:
                        ...
                        if args.dataset == 'imagenet':
                            ...
                            # len_sub = int(0.8 * total_len)
                            # train_subset, _ = random_split(train_dataset, [len_sub, total_len-len_sub])
                    ...
                    # net_last_layer = net.model_to_last_layer[args.model]
                ...
                (_, _, _), (_, _, _), ood_m1_auroc, ood_m1_auprc = get_roc_auc_logits(mars, ood_mars, self_consistency_o, device)
                (_, _, _), (_, _, _), ood_m2_auroc, ood_m2_auprc = get_roc_auc_logits(logits, ood_logits, entropy, device)
                ...
                (_, _, _), (_, _, _), err_m1_auroc, err_m1_auprc = get_roc_auc_logits(mars_right, mars_wrong, self_consistency_o, device)
                ...
                (_, _, _), (_, _, _), adv_m1_auroc, adv_m1_auprc = get_roc_auc_logits(mars, adv_mars,self_consistency_o, device)
                (_, _, _), (_, _, _), adv_m2_auroc, adv_m2_auprc = get_roc_auc_logits(logits, adv_logits, entropy, device)
                ...
                if args.sample_noise:
                    ...
                    for idx_ep, ep in enumerate(adv_eps):
                        ...
                        uncertainties = self_consistency_e(adv_mars).detach().cpu().numpy()
                        quantiles = np.quantile(uncertainties, np.linspace(0, 1, 10))
                        quantiles = np.delete(quantiles, 0)
                        unc_list = []
                        accuracy_list = []
                        ...
                        (_, _, _), (_, _, _), adv_m1_auroc, adv_m1_auprc = get_roc_auc_logits(mars, adv_mars, self_consistency_e,device)
                        (_, _, _), (_, _, _), adv_m2_auroc, adv_m2_auprc = get_roc_auc_logits(logits, adv_logits, entropy,device)
                        print('adv_m1_auroc,adv_m2_auroc',adv_m1_auroc,adv_m2_auroc)
    ...
    if args.sample_noise:
        ...
        # for i, (m, s) in enumerate(zip(mean_unc, std_unc)):
        #     wandb.log({
        #         "Uncertainty": m,
        #         "noise_step": i
        #     }, step=i)
