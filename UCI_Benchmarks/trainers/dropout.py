...
class Dropout:
    ...
    def evaluate(self, x, y, y_mu, y_scale):
        self.model.eval()
        deterministic_mu, _ = self.model(x)
        self.model.train()

        tic = time.time()
        dropout_mus, dropout_sigmas = [], []
        with torch.no_grad():
            for _ in range(self.mc_samples):
                mu, sigma = self.model(x)
                dropout_mus.append(mu)
                dropout_sigmas.append(sigma)

        stacked_mus = torch.stack(dropout_mus, dim=0)
        stacked_sigmas = torch.stack(dropout_sigmas, dim=0)
        ...
        sigma_dropout = (mean_sigma.detach().cpu().numpy() * y_scale).squeeze()
        ...
        ece = ece_pi(y, predictions - 2 * sigma_dropout, predictions + 2 * sigma_dropout)
        is_score = interval_score(y, predictions, predictions - 2 * sigma_dropout, predictions + 2 * sigma_dropout, alpha=0.05)
        mpi_width = np.mean(4 * sigma_dropout)
        within_interval = ((y >= (predictions - 2 * sigma_dropout)) & (y <= (predictions + 2 * sigma_dropout))).astype(float)
        picp = np.mean(within_interval)

        within_interval_down = (y[predictions > y] >= (predictions[predictions > y] - 2 * sigma_dropout[predictions > y])).astype(float)
        picp_down = np.mean(within_interval_down)
        within_interval_up = (y[predictions < y] <= (predictions[predictions < y] + 2 * sigma_dropout[predictions < y])).astype(float)
