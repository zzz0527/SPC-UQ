...
class DenseNormalGamma(nn.Module):
    ...
    def forward(self, x):
        ...
        alpha = self.evidence(logalpha) + 1  # 确保 alpha >= 1
        beta = self.evidence(logbeta)
        return torch.cat([mu, v, alpha, beta], dim=-1)


class Evidential_net(nn.Module):
    def __init__(self, input_shape, num_neurons=50, num_layers=1, activation=nn.ReLU()):
        super(Evidential_net, self).__init__()
        layers = []
        ...
        layers.append(DenseNormalGamma(in_features=num_neurons, units=1))  # 输出 mu 和 sigma
